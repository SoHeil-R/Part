local TIME_CHECK = 2
local function pre_process(msg)
local data = load_data(_config.moderation.data)
local chat = msg.to.id
local user = msg.from.id
local is_channel = msg.to.type == "channel"
local is_chat = msg.to.type == "chat"
local auto_leave = 'auto_leave_bot'
   if is_channel or is_chat then
           if data[tostring(chat)] then
          if data[tostring(chat)]['settings']['time_check'] then
            TIME_CHECK = tonumber(data[tostring(chat)]['settings']['time_check'])
          end
        end
    if msg.text then
  if msg.text:match("(.*)") then
    if not data[tostring(msg.to.id)] and not redis:get(auto_leave) and not is_admin(msg) then
  tdcli.sendMessage(msg.to.id, "", 0, "لطفا برای خرید گروه با پشتبانی تماس بگیرید:\n@So8eil\n@SoHeilDktaBot\n@PartTeam", 0, "md")
  tdcli.changeChatMemberStatus(chat, our_id, 'Left', dl_cb, nil)
      end
   end
end
    if data[tostring(chat)] and data[tostring(chat)]['mutes'] then
		mutes = data[tostring(chat)]['mutes']
	else
		return
	end
	if mutes.mute_all then
		mute_all = mutes.mute_all
	else
		mute_all = 'no'
	end
	if mutes.mute_gif then
		mute_gif = mutes.mute_gif
	else
		mute_gif = 'no'
	end
   if mutes.mute_photo then
		mute_photo = mutes.mute_photo
	else
		mute_photo = 'no'
	end
	if mutes.mute_contact then
		mute_contact = mutes.mute_contact
	else
		mute_contact = 'no'
	end
	if mutes.mute_inline then
		mute_inline = mutes.mute_inline
	else
		mute_inline = 'no'
	end
	if mutes.mute_game then
		mute_game = mutes.mute_game
	else
		mute_game = 'no'
	end
	if mutes.mute_text then
		mute_text = mutes.mute_text
	else
		mute_text = 'no'
	end
	if mutes.mute_keyboard then
		mute_keyboard = mutes.mute_keyboard
	else
		mute_keyboard = 'no'
	end
	if mutes.mute_location then
		mute_location = mutes.mute_location
	else
		mute_location = 'no'
	end
   if mutes.mute_document then
		mute_document = mutes.mute_document
	else
		mute_document = 'no'
	end
	if mutes.mute_voice then
		mute_voice = mutes.mute_voice
	else
		mute_voice = 'no'
	end
	if mutes.mute_audio then
		mute_audio = mutes.mute_audio
	else
		mute_audio = 'no'
	end
	if mutes.mute_video then
		mute_video = mutes.mute_video
	else
		mute_video = 'no'
	end
	if data[tostring(chat)] and data[tostring(chat)]['settings'] then
		settings = data[tostring(chat)]['settings']
	else
		return
	end
	if settings.lock_link then
		lock_link = settings.lock_link
	else
		lock_link = 'no'
	end
	if settings.lock_link then
		lock_link_ban = settings.lock_link
	else
		lock_link_ban = 'ban'
	end
    if settings.lock_link then
		lock_link_warn = settings.lock_link
	else
		lock_link_warn = 'warn'
	end
	if settings.lock_tag then
		lock_tag = settings.lock_tag
	else
		lock_tag = 'no'
	end
	if settings.lock_tag then
		lock_tag_ban = settings.lock_tag
	else
		lock_tag_ban = 'ban'
	end
	if settings.lock_tag then
		lock_tag_warn = settings.lock_tag
	else
		lock_tag_warn = 'warn'
	end
	if settings.lock_pin then
		lock_pin = settings.lock_pin
	else
		lock_pin = 'no'
	end
	if settings.lock_arabic then
		lock_arabic_ban = settings.lock_arabic
	else
		lock_arabic_ban = 'ban'
	end
	if settings.lock_arabic then
		lock_arabic_warn = settings.lock_arabic
	else
		lock_arabic_warn = 'warn'
	end
	if settings.lock_arabic then
		lock_arabic = settings.lock_arabic
	else
		lock_arabic = 'no'
	end
	if settings.lock_sticker then
		lock_sticker_ban = settings.lock_sticker
	else
		lock_sticker_ban = 'ban'
	end
    if settings.lock_sticker then
		lock_sticker_warn = settings.lock_sticker
	else
		lock_sticker_warn = 'warn'
	end
    if settings.lock_sticker then
		lock_sticker = settings.lock_sticker
	else
		lock_sticker = 'no'
	end
    if settings.lock_sticker then
		lock_sticker_ban = settings.lock_sticker
	else
		lock_sticker_ban = 'ban'
	end
    if settings.lock_sticker then
		lock_sticker_warn = settings.lock_sticker
	else
		lock_sticker_warn = 'warn'
	end
	if settings.lock_mention then
		lock_mention = settings.lock_mention
	else
		lock_mention = 'no'
	end
	if settings.lock_mention then
		lock_mention_ban = settings.lock_mention
	else
		lock_mention_ban = 'ban'
	end
	if settings.lock_mention then
		lock_mention_warn = settings.lock_mention
	else
		lock_mention_warn = 'warn'
	end
    if settings.lock_cmd then
		lock_cmd = settings.lock_cmd
	else
		lock_cmd = 'no'
	end
	if settings.lock_cmd then
		lock_cmd_ban = settings.lock_cmd
	else
		lock_cmd_ban = 'ban'
	end
    if settings.lock_cmd then
		lock_cmd_warn = settings.lock_cmd
	else
		lock_cmd_warn = 'warn'
	end
	if settings.lock_fwd then
		lock_fwd = settings.lock_fwd
	else
		lock_fwd = 'no'
	end
	if settings.lock_fwd then
		lock_fwd_ban = settings.lock_fwd
	else
		lock_fwd_ban = 'ban'
	end
	if settings.lock_fwd then
		lock_fwd_warn = settings.lock_fwd
	else
		lock_fwd_warn = 'warn'
	end
		if settings.lock_edit then
		lock_edit = settings.lock_edit
	else
		lock_edit = 'no'
	end
		if settings.lock_spam then
		lock_spam = settings.lock_spam
	else
		lock_spam = 'no'
	end
	if settings.flood then
		lock_flood = settings.flood
	else
		lock_flood = 'no'
	end
	if settings.lock_markdown then
		lock_markdown = settings.lock_markdown
	else
		lock_markdown = 'no'
	end
    if settings.lock_markdown then
		lock_markdown_ban = settings.lock_markdown
	else
		lock_markdown_ban = 'ban'
	end
    if settings.lock_markdown then
		lock_markdown_warn = settings.lock_markdown
	else
		lock_markdown_warn = 'warn'
	end
	if settings.lock_webpage then
		lock_webpage = settings.lock_webpage
	else
		lock_webpage = 'no'
	end
    if settings.lock_webpage then
		lock_webpage_ban = settings.lock_webpage
	else
		lock_webpage_ban = 'ban'
	end
    if settings.lock_webpage then
		lock_webpage_warn = settings.lock_webpage
	else
		lock_webpage_warn = 'warn'
	end
	if settings.lock_tgser then
		lock_tgser = settings.lock_tgser
	else
		lock_tgser = 'no'
	end
if msg.adduser or msg.joinuser or msg.deluser then
  if lock_tgser == "yes" then
del_msg(chat, tonumber(msg.id))
  end
end

   if msg.pinned and is_channel then
  if lock_pin == "yes" then
     if is_owner(msg) then
      return
     end
     if tonumber(msg.from.id) == our_id then
      return
     end
    local pin_msg = data[tostring(chat)]['pin']
      if pin_msg then
  tdcli.pinChannelMessage(msg.to.id, pin_msg, 1)
       elseif not pin_msg then
   tdcli.unpinChannelMessage(msg.to.id)
          end
     tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر : '..('@'..msg.from.username or '404')..'\nشما اجازه دسترسی به سنجاق پیام را ندارید، به همین دلیل پیام قبلی مجدد سنجاق میگردد'..part, 0, "html")
      end
  end
      if not is_mod(msg) then
if msg.edited and lock_edit == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
    end
  end
if msg.forward_info_ and lock_fwd == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
    end
  end
if msg.forward_info_ and lock_fwd_ban == "ban" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
    end
  end
if msg.forward_info_ and lock_fwd_warn == "warn" then
 if is_channel then
 local warn_forward = 'warn_forward:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_forward)
 local forward_redis = redis:get(warn_forward)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(forward_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(forward_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(forward_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_forward = 'warn_forward:'..msg.to.id..':'..msg.from.id
 redis:set(warn_forward, 0)-- Reset the Counter
    end
  end
end
if msg.photo_ and mute_photo == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.video_ and mute_video == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.document_ and mute_document == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.sticker_ and lock_sticker == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
if msg.sticker_ and lock_sticker_ban == "ban" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.sticker_ and lock_sticker_warn == "warn" then
 if is_channel then
 local warn_sticker = 'warn_sticker:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_sticker)
 local sticker_redis = redis:get(warn_sticker)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(sticker_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(sticker_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(sticker_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_sticker = 'warn_sticker:'..msg.to.id..':'..msg.from.id
 redis:set(warn_sticker, 0)-- Reset the Counter
    end
  end
end
    if msg.animation_ and mute_gif == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.contact_ and mute_contact == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.location_ and mute_location == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.voice_ and mute_voice == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
   if msg.content_ and mute_keyboard == "yes" then
  if msg.reply_markup_ and  msg.reply_markup_.ID == "ReplyMarkupInlineKeyboard" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
      end
   end
end
    if tonumber(msg.via_bot_user_id_) ~= 0 and mute_inline == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.game_ and mute_game == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
    if msg.audio_ and mute_audio == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
if msg.text then
			local _nl, ctrl_chars = string.gsub(msg.text, '%c', '')
        local max_chars = 40
        if data[tostring(msg.to.id)] then
          if data[tostring(msg.to.id)]['settings']['set_char'] then
            max_chars = tonumber(data[tostring(msg.to.id)]['settings']['set_char'])
          end
        end
			 local _nl, real_digits = string.gsub(msg.text, '%d', '')
			local max_real_digits = tonumber(max_chars) * 50
			local max_len = tonumber(max_chars) * 51
			if lock_spam == "yes" then
			if string.len(msg.text) > max_len or ctrl_chars > max_chars or real_digits > max_real_digits then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
      end
   end
end
end

if msg.media.caption then
local link_caption = msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.media.caption:match("[Tt].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
if link_caption
and lock_link == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end

local link_caption = msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.media.caption:match("[Tt].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
if link_caption
and lock_link_ban == "ban" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
   end
end

local link_caption = msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.media.caption:match("[Tt].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
if link_caption
and lock_link_warn == "warn" then
 if is_channel then
 local warn_links = 'warn_links:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_links)
 local links_redis = redis:get(warn_links)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(links_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(links_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(links_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_links = 'warn_links:'..msg.to.id..':'..msg.from.id
 redis:set(warn_links, 0)-- Reset the Counter
    end
  end
end

local tag_caption = msg.media.caption:match("@") or msg.media.caption:match("#")
if tag_caption and lock_tag == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end

local tag_caption = msg.media.caption:match("@") or msg.media.caption:match("#")
if tag_caption and lock_tag_ban == "ban" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
   end
end

local tag_caption = msg.media.caption:match("@") or msg.media.caption:match("#")
if tag_caption and lock_tag_warn == "warn" then
 if is_channel then
 local warn_tags = 'warn_tags:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_tags)
 local tags_redis = redis:get(warn_tags)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(tags_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(tags_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(tags_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_tags = 'warn_tags:'..msg.to.id..':'..msg.from.id
 redis:set(warn_tags, 0)-- Reset the Counter
    end
  end
end

if is_filter(msg, msg.media.caption) then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
      end
    end
local arabic_caption = msg.media.caption:match("[\216-\219][\128-\191]")
if arabic_caption and lock_arabic == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
      end
   end
end
end
if msg.text then
			local _nl, ctrl_chars = string.gsub(msg.text, '%c', '')
			 local _nl, real_digits = string.gsub(msg.text, '%d', '')
			if lock_spam == "yes" then
   if string.len(msg.text) > 2049 or ctrl_chars > 40 or real_digits > 2000 then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
      end
   end
end
 if lock_cmd == "yes" and not is_mod(msg) then
  if  msg.text:match('(.*)') then
      return false
  end
  end
local link_msg = msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.text:match("[Tt].[Mm][Ee]/") or msg.text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
if link_msg and lock_link == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
local link_msg = msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.text:match("[Tt].[Mm][Ee]/") or msg.text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
if link_msg and lock_link_ban == "ban" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
   end
end

local link_msg = msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.text:match("[Tt].[Mm][Ee]/") or msg.text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
if link_msg and lock_link_warn == "warn" then
 if is_channel then
 local warn_link = 'warn_link:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_link)
 local link_redis = redis:get(warn_link)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(link_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(link_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(link_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_link = 'warn_link:'..msg.to.id..':'..msg.from.id
 redis:set(warn_link, 0)-- Reset the Counter
    end
  end
end
local tag_msg = msg.text:match("@") or msg.text:match("#")
if tag_msg and lock_tag == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end

local tag_msg = msg.text:match("@") or msg.text:match("#")
if tag_msg and lock_tag_ban == "ban" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
   end
end

local tag_msg = msg.text:match("@") or msg.text:match("#")
if tag_msg and lock_tag_warn == "warn" then
 if is_channel then
 local warn_tag = 'warn_tag:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_tag)
 local tag_redis = redis:get(warn_tag)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(tag_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(tag_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(tag_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_tag = 'warn_tag:'..msg.to.id..':'..msg.from.id
 redis:set(warn_tag, 0)-- Reset the Counter
    end
  end
end
if is_filter(msg, msg.text) then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
      end
    end
local arabic_msg = msg.text:match("[\216-\219][\128-\191]")
if arabic_msg and lock_arabic == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end

if msg.text:match("(.*)")
and mute_text == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
     end
   end
end
if mute_all == "yes" then 
 if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
   end
end
if msg.content_.entities_ and msg.content_.entities_[0] then
    if msg.content_.entities_[0].ID == "MessageEntityMentionName" then
      if lock_mention == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
  del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
             end
          end
      end
end
	  if msg.content_.entities_ and msg.content_.entities_[0] then
    if msg.content_.entities_[0].ID == "MessageEntityMentionName" then
      if lock_mention_ban == "ban" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
             end
          end
      end
end
	  	  if msg.content_.entities_ and msg.content_.entities_[0] then
    if msg.content_.entities_[0].ID == "MessageEntityMentionName" then
      if lock_mention_warn == "warn" then
 if is_channel then
 local warn_mention = 'warn_mention:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_mention)
 local mention_redis = redis:get(warn_mention)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(mention_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(mention_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(mention_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_mention = 'warn_mention:'..msg.to.id..':'..msg.from.id
 redis:set(warn_mention, 0)-- Reset the Counter
    end
  end
end
  if msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
      if lock_webpage == "yes" then
if is_channel then
 del_msg(chat, tonumber(msg.id))
 del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
             end
          end
      end
	    if msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
      if lock_webpage_ban == "ban" then
if is_channel then
 del_msg(chat, tonumber(msg.id))
kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
             end
          end
      end
	    if msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
      if lock_webpage == "warn" then
if is_channel then
 local warn_webpage = 'warn_webpage:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_webpage)
 local webpage_redis = redis:get(warn_webpage)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(webpage_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(webpage_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(webpage_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_webpage = 'warn_webpage:'..msg.to.id..':'..msg.from.id
 redis:set(warn_webpage, 0)-- Reset the Counter
    end
  end
end
  if msg.content_.entities_[0].ID == "MessageEntityBold" or msg.content_.entities_[0].ID == "MessageEntityCode" or msg.content_.entities_[0].ID == "MessageEntityPre" or msg.content_.entities_[0].ID == "MessageEntityItalic" then
      if lock_markdown == "yes" then
if is_channel then
 del_msg(chat, tonumber(msg.id))
  del_msg(chat, tonumber(msg.id))
  elseif is_chat then
kick_user(user, chat)
             end
          end
      end
	if msg.content_.entities_[0].ID == "MessageEntityBold" or msg.content_.entities_[0].ID == "MessageEntityCode" or msg.content_.entities_[0].ID == "MessageEntityPre" or msg.content_.entities_[0].ID == "MessageEntityItalic" then
      if lock_markdown_ban == "ban" then
if is_channel then
 del_msg(chat, tonumber(msg.id))
kick_user(user, chat)
  elseif is_chat then
kick_user(user, chat)
             end
          end
      end
if msg.content_.entities_[0].ID == "MessageEntityBold" or msg.content_.entities_[0].ID == "MessageEntityCode" or msg.content_.entities_[0].ID == "MessageEntityPre" or msg.content_.entities_[0].ID == "MessageEntityItalic" then
      if lock_markdown_warn == "warn" then
if is_channel then
 local warn_markdown = 'warn_markdown:'..msg.to.id..':'..msg.from.id
 redis:incr(warn_markdown)
 local markdown_redis = redis:get(warn_markdown)
 if msg.from.username then 
 user = "@"..msg.from.username
 else 
 user = msg.from.first_name
 end
 if tonumber(markdown_redis) == 1 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\nتعداد اخطارات: <b>[1/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(markdown_redis) == 2 and not is_mod(msg)then
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز یک اخطا دریافت کرد\n#درصورت ارسال مجدد از گروه محروم خواهید شد\nتعداد اخطارات: <b>[2/3]</b>'..part, 0, "html")
 del_msg(chat, tonumber(msg.id))
end
if strict == "warn" or tonumber(markdown_redis) == 3 and not is_mod(msg)then
 del_msg(chat, tonumber(msg.id))
kick_user(msg.from.id, msg.to.id)
tdcli.sendMessage(msg.to.id, msg.id, 0, 'کاربر :['..(user or '404')..']\nبه دلیل ارسال رسانه غیر مجاز از گروه محروم شد\nوضعیت: [اخراج]\n تعداد اخطارات: <b>[3/3]</b>'..part, 0, "html")
 local warn_markdown = 'warn_markdown:'..msg.to.id..':'..msg.from.id
 redis:set(warn_markdown, 0)-- Reset the Counter
    end
  end
end
 end
if msg.to.type ~= 'pv' then
  if lock_flood == "yes" then
    local hash = 'user:'..user..':msgs'
    local msgs = tonumber(redis:get(hash) or 0)
        local NUM_MSG_MAX = 5
        if data[tostring(chat)] then
          if data[tostring(chat)]['settings']['num_msg_max'] then
            NUM_MSG_MAX = tonumber(data[tostring(chat)]['settings']['num_msg_max'])
          end
        end
    if msgs > NUM_MSG_MAX then
  if is_mod(msg) then
    return
  end
  if msg.adduser and msg.from.id then
    return
  end
   if msg.from.username then
      user_name = "@"..msg.from.username
         else
      user_name = msg.from.first_name
     end
if redis:get('sender:'..user..':flood') then
return
else
   del_msg(chat, msg.id)
    kick_user(user, chat)
   if not lang then
  tdcli.sendMessage(chat, msg.id, 0, "_User_ "..user_name.." `[ "..user.." ]` _has been_ *kicked* _because of_ *flooding*"..part, 0, "md")
   elseif lang then
  tdcli.sendMessage(chat, msg.id, 0, "_کاربر_ "..user_name.." `[ "..user.." ]` _به دلیل ارسال پیام های مکرر اخراج شد_"..part, 0, "md")
    end
redis:setex('sender:'..user..':flood', 30, true)
      end
    end
    redis:setex(hash, TIME_CHECK, msgs+1)
               end
           end
      end
   end
end
end
end
return {
	patterns = {},
	pre_process = pre_process
}
--BY: @So8eil
